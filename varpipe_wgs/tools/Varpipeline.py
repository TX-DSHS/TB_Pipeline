...
class snp():
    ...
    def __init__(self, input, outdir, reference, name, paired,input2, verbose, project, argString):
        i                       = datetime.now()
        self.name               = name
        #self.flog               = "Output_" + i.strftime('%m_%d_%Y')
        self.flog               = os.path.join("Output", project)
    ...
    def runCoverage(self):
        ...
        for lines in fh20:
            if lines.startswith('Sample ID'):
               continue
            fields = lines.rstrip("\r\n").split("\t")
            if float(fields[2]) < 90.0 or int(fields[3]) < 50 or float(fields[4]) < 90.0:
